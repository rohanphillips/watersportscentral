class CommentsController < ApplicationController
  def create
    case comment_params[:from_controller]
      when "locations"
        @comment = Location.find(comment_params[:location_id]).comments.create(user_id: comment_params[:user_id], comment_text: comment_params[:comment_text])
...
private
  ...
  def comment_params
    params.require(:comment).permit(:location_id, :event_id, :user_id, :comment_text, :from_controller)
  end
end